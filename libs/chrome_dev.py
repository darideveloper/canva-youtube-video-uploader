...
class ChromDevWrapper():
    ...
    def __init__(self, chrome_path: str = "", port: int = 9222,
        ...
        # Calculate default chrome path if not provided
        if chrome_path == "":
    ...
    def get_cookies(self) -> list:
        ...
        sleep(self.base_wait_time)
        return cookies[0]["result"]["cookies"]
    ...
    def send_data(self, selector: str, data: str):
        ...
        result = self.chrome.DOM.querySelector(
            nodeId=element, selector=selector)
    ...
    def get_attrib(self, selector: str, attrib: str) -> str:
        ...
        script = f"""document.querySelector(`{selector}`).getAttribute(`{
            attrib}`)"""
        response = self.chrome.Runtime.evaluate(expression=script)
        try:
            return response[0]['result']["result"]["value"].strip()
    ...
    def send_input_file(self, selector: str, file_path: str):
        ...
        input_file_node = self.chrome.DOM.querySelector(
            nodeId=self.chrome.DOM.getDocument(
            )[0]["result"]["root"]["nodeId"],
            selector=selector
        )[0]["result"]["nodeId"]
    ...
    def get_current_url(self) -> str:
        """ Get current url with js and return it
